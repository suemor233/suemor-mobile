...
const ArchiveItem: FC<Post> = ({ _id, title, created, tags }) => {
  return (
    <Link asChild href={`posts/${_id}`}>
      <TouchableOpacity className="bg-white rounded-md dark:bg-zinc-900 shadow-md p-2 space-y-1">
        ...
        <View className="flex-row space-x-1">
          ...
          <View className="flex-row space-x-2 overflow-hidden">
            {tags.map((tag, index) => (
              <Text
                key={index}
                className="text-gray-400  bg-gray-100 dark:bg-zinc-800 text-sm dark:text-blue-400"
              >
                {tag}
              </Text>
            ))}
